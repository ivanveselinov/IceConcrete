...
const Header = () => {
  ...
  // <Fade top duration={3000}>
    <div className="z-30 w-full fixed h-32 inline-flex border-r justify-between bg-orange-600 rounded-t-xl xs:block sm:block md:inline-flex"> 
        ...
        <div className="text-center flex m-auto space-x-20 xs:w-3/4 xs:space-x-14 xs:text-sm xs:mt-10 md:mt-10 md:space-x-20 md:w-1/2 md:text-xl xl:text-2xl xl:w-1/2 "> 
        ...
        <button className="block w-1/7 space-x-1 xs:text-sm xs:flex xs:space-x-24 xs:m-auto space-x-auto sm:text-sm sm:flex md:mt-14 md:block" on onClick={signOutUser}>
               <p className="p-1 mt-2 text-2xl xs:text-sm sm:text-sm md:hidden">Welcome Back</p>
               <p className="p-1 mt-2 text-2xl xs:text-sm  sm:text-sm md:hidden">{appUser.email}</p>
          <p className="p-1 text-xl mt-1 mr-2  xs:text-sm sm:text-sm md:text-xl ">Logout<LogoutIcon/></p>
        </button>
}
